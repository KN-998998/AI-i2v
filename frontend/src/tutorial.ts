...
const stepContent: Array<{ route: WorkflowRoute; title: string; description: string; screenshot: string; bullets: string[]; checkpoint: string; warning: string }> = [
  ...
  { route: "/workflow/image-processing", title: "图片处理", description: "将菜品主体与背景模板合成视频首帧；人物或手部素材可以保留原图。", screenshot: "step-2-image-processing.png", bullets: ["普通菜品先选择背景模板，再执行抠图与合成。", "人物、厨师或手部素材选择保留原图模式。", "检查处理结果后再进入提示词装配。"], checkpoint: "处理节点出现有效的处理后首帧。", warning: "点击节点卡片上的其他按钮不会解锁本步骤，必须完成真实图片处理。" },
  { route: "/workflow/prompts", title: "提示词装配", description: "用 L0、L1、L2 槽位描述画面元素、主运动和次级动态，生成可校验的 Kling 提示词。", screenshot: "step-3-prompts.png", bullets: ["先勾选画面中确实存在的元素。", "L1 只选择一个主运动对象和动作。", "L2 最多选择两项，并处理冲突提示。"], checkpoint: "提示词校验通过并点击实时装配。", warning: "实时装配只在素材上传和图片处理完成后可用，不会因为点击其他节点按钮而提前解锁。" },
  ...
  { route: "/workflow/sound", title: "声音与文字", description: "为无声成片配置 BGM、人声和时间轴文字，生成最终有声视频。", screenshot: "step-6-sound.png", bullets: ["上传或选择 BGM，并调整音量。", "按时间段配置人声和画面文字。", "确认字幕位置、字体和语音绑定关系。"], checkpoint: "声音与文字合成任务完成。", warning: "声音步骤必须建立在无声成片完成之后，不能直接从提示词步骤跳入。" },
