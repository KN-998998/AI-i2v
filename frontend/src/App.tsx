...
function CanvasPage({ onToast }: { onToast: (message: string) => void }) {
  ...
  return <><main className={`main-column canvas-main ${hasSelection ? "is-inspecting" : ""}`}><div className="canvas-toolbar"><div><span className="panel-label">CANVAS OVERVIEW</span><strong>流程画布总览</strong></div><div className="toolbar-actions"><div className="add-menu"><button type="button" className="btn btn-primary" onClick={() => setAddOpen(value => !value)}>＋ 添加节点</button>{addOpen && <div className="add-menu-pop">{(["input", "image_process", "prompt", "generator", "output", "sound", "custom"] as NodeKind[]).map(kind => <button type="button" key={kind} onClick={() => add(kind)}>{nodeCatalog[kind].title}</button>)}</div>}</div><button type="button" className="btn" onClick={() => { deleteSelected(); onToast("已删除选中对象"); }}>删除选中</button></div></div><div className="flow-shell flow-overview"><ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={setNodes} onEdgesChange={setEdges} onConnect={onConnect} onSelectionChange={onSelectionChange} onPaneClick={() => setSelection(null)} onInit={instance => { window.setTimeout(() => instance.fitView({ padding: 0.18, minZoom: 0.45, maxZoom: 1.2 }), 0); }} fitView fitViewOptions={{ padding: 0.18, minZoom: 0.45, maxZoom: 1.2 }} minZoom={0.05} maxZoom={8} deleteKeyCode={["Backspace", "Delete"]} proOptions={{ hideAttribution: true }}><Background color="#2b3438" gap={24} size={1} /><Controls position="bottom-right" /><MiniMap position="top-right" nodeColor="#355e62" maskColor="rgba(10,14,16,.26)" /></ReactFlow></div><div className="overview-help"><strong>按步骤进入独立页面</strong><span>画布只负责查看和连接流程；图片处理、片段生成、排序、合成和声音配置在左侧对应页面完成。</span></div></main>{hasSelection && <Inspector onToast={onToast} />}</>;
}
