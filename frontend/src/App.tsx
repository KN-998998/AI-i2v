...
function App() {
  ...
  const workflowProgress = useMemo(() => deriveWorkflowProgress(nodes, candidateClips, composeWorkspaces, edges), [nodes, edges, candidateClips, composeWorkspaces]);
...
function CanvasPageLegacy({ onToast }: { onToast: (message: string) => void }) {
  ...
  const [pointerMode, setPointerMode] = useState<"pan" | "select">("pan");
  ...
  const selectedObjectCount = nodes.filter(node => node.selected).length + edges.filter(edge => edge.selected).length;
  const displayEdges = edges.map(edge => edge.id === freshEdgeId ? { ...edge, animated: true, className: `${edge.className ?? ""} edge-fresh` } : edge);
  return <><main className="main-column canvas-main"><div className="canvas-toolbar"><div><span className="panel-label">CANVAS OVERVIEW</span><strong>流程画布总览</strong></div><div className="toolbar-actions"><div className="toolbar-actions toolbar-mode" role="group" aria-label="鼠标模式"><span>鼠标模式</span><button type="button" className={`btn ${pointerMode === "pan" ? "btn-primary" : ""}`} aria-pressed={pointerMode === "pan"} onClick={() => setPointerMode("pan")}>拖动</button><button type="button" className={`btn ${pointerMode === "select" ? "btn-primary" : ""}`} aria-pressed={pointerMode === "select"} onClick={() => setPointerMode("select")}>框选</button></div><button type="button" className="btn" onClick={() => void arrange()}>整理流程</button><div className="add-menu"><button type="button" className="btn btn-primary" onClick={() => setAddOpen(value => !value)}>＋ 添加节点</button>{addOpen && <div className="add-menu-pop">{(["input", "image_process", "prompt", "generator", "output", "sound", "custom"] as NodeKind[]).map(kind => <button type="button" key={kind} onClick={() => add(kind)}>{nodeCatalog[kind].title}</button>)}</div>}</div><button type="button" className="btn" onClick={() => { const deleted = deleteSelected(); onToast(deleted ? "已删除选中对象" : selectedNodeId ? "流程核心节点不能删除" : selectedEdgeId ? "选中的连接不存在" : "请先选择要删除的节点或连接"); }}>删除选中{selectedObjectCount ? `（${selectedObjectCount}）` : ""}</button></div></div><div className={`flow-shell flow-overview ${isConnecting ? "is-connecting" : ""}`}><ReactFlow nodes={nodes} edges={displayEdges} nodeTypes={nodeTypes} onNodesChange={setNodes} onEdgesChange={setEdges} onConnect={onConnect} onConnectStart={() => setIsConnecting(true)} onConnectEnd={() => setIsConnecting(false)} onSelectionChange={onSelectionChange} onPaneClick={() => setSelection(null)} onInit={instance => { window.setTimeout(() => instance.fitView({ padding: 0.18, minZoom: 0.45, maxZoom: 1.2 }), 0); }} fitView fitViewOptions={{ padding: 0.18, minZoom: 0.45, maxZoom: 1.2 }} connectionLineType={ConnectionLineType.SmoothStep} connectionLineStyle={{ stroke: "#8ee5dc", strokeWidth: 2.5, strokeDasharray: "7 5" }} defaultEdgeOptions={{ type: "smoothstep", markerEnd: { type: MarkerType.ArrowClosed, color: "#718287", width: 15, height: 15 } }} minZoom={0.05} maxZoom={8} panOnDrag={pointerMode === "pan"} nodesDraggable={pointerMode === "pan"} selectionOnDrag={pointerMode === "select"} deleteKeyCode={["Backspace", "Delete"]} proOptions={{ hideAttribution: true }}><Background color="#2b3438" gap={24} size={1} /><Controls position="bottom-right" /><MiniMap position="top-right" nodeColor="#355e62" maskColor="rgba(10,14,16,.26)" /></ReactFlow></div><div className="overview-help"><strong>按步骤进入独立页面</strong><span>切换到“框选”后，在画布空白处拖出选区，再用“删除选中”或键盘 Delete 批量删除；流程核心节点始终受保护。</span></div></main>{hasSelection && <Inspector onToast={onToast} />}</>;
}
...
function CanvasWorkspace({ onToast }: { onToast: (message: string) => void }) {
  ...
  const progress = useMemo(() => deriveWorkflowProgress(nodes, candidateClips, composeWorkspaces, edges), [nodes, edges, candidateClips, composeWorkspaces]);
