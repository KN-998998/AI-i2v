...
export function ImageProcessingPage({ onToast }: { onToast: (message: string) => void }) {
  ...
  return <main className="step-main">
    ...
    <div className="step-page-grid"><div className="step-page-main">
      ...
      <div className="step-context"><button type="button" className="btn btn-primary" disabled={!node.data.processedImagePreview} onClick={() => navigate("/workflow/prompts")}>进入提示词装配</button><span className="muted">草稿：{draftId} · 状态：{node.data.status}</span></div>
