...
import { isTrimConfirmed, bgmLabel, bgmModeFor, type BgmMode, overlayCoordinatesFromItem, overlayStyleFromItem, type OverlayItem, type TimelineClip, type VoiceItem } from "../model";
...
export function StoryboardTimeline({ clips, mode = "sound", overlayItems, onOverlayFocus, onUpdateOverlay, onRemoveOverlay, onUpdateClip, voiceItems = [], voiceText, onVoiceFocus, onUpdateVoice, onRemoveVoice, bgmName, bgmMode, bgmTrack, onRemoveBgm }: StoryboardTimelineProps) {
  ...
  return <section className={`storyboard-editor ${showSoundTracks ? "storyboard-editor-sound" : "storyboard-editor-clip"}`}>
    ...
    <div className="storyboard-scroll"><div className="storyboard-track-stack" style={{ minWidth: `${trackWidth}px` }}>
      ...
      {showSoundTracks && <StoryboardTrack label="BGM"><AudioTrackBlock className="bgm-track-block" label={`BGM · ${bgmLabel({ bgmMode, bgmName, bgmTrack })}`} tone="bgm" total={total} onRemove={bgmModeFor({ bgmMode, bgmName }) === "none" ? undefined : onRemoveBgm} /></StoryboardTrack>}
