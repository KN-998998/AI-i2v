...
export function Pipeline({ path, collapsed, onToggle }: { path: WorkflowRoute; collapsed: boolean; onToggle: () => void }) {
  ...
  return <aside id="pipeline-navigation" className="pipeline" aria-label="生产工作台导航">
    ...
    {workflowRoutes.filter(item => item.path !== "/workflow/output").map((item, index) => <div className="pipeline-step-row" key={item.path}><PipelineItem item={item} active={path === item.path} progress={progress.steps[index]} stepIndex={index} /><PipelineTutorialButton item={item} /></div>)}
    <div className="pipeline-divider" />
    <div className="pipeline-group-label output-label"><span>07</span><span>交付</span></div>
    <div className="pipeline-step-row"><PipelineItem item={workflowRoutes[workflowRoutes.length - 1]} active={path === "/workflow/output"} progress={progress.steps[workflowRoutes.length - 1]} stepIndex={workflowRoutes.length - 1} /><PipelineTutorialButton item={workflowRoutes[workflowRoutes.length - 1]} /></div>
