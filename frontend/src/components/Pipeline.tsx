...
export function Pipeline({ path }: { path: WorkflowRoute }) {
  return <aside className="pipeline">
    ...
    <button type="button" className={`task-center-link ${path === "/workflow/weekly-plan" ? "active" : ""}`} onClick={() => navigate("/workflow/weekly-plan")}><span className="task-center-link-icon">周</span><span><strong>周计划生产</strong><small>配置自动选材与生成计划</small></span></button>
