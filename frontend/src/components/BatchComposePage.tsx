...
export function BatchComposePage({ onToast }: Props) {
  ...
  return <main className="step-main"><div className="step-breadcrumb"><button type="button" className="link-button" onClick={() => navigate("/canvas-mvp")}>流程画布</button><span>/</span><strong>成片合成</strong></div><div className="step-header"><div><span className="panel-label">WORKFLOW STEP</span><h1>成片合成</h1><p>从前面生成节点汇入的候选片段池中，先确定每个片段的精彩区间，再批量生成多套可人工调整顺序的无声成片方案。</p></div></div><section className="step-panel batch-compose-controls"><div><span className="panel-label">BATCH COMPOSE</span><h2>{batchCount} 条成片 · 每条 {clipCount} 个片段</h2><p className="muted">候选片段 {candidates.length} 个；智能推荐按质量分和菜品多样性选取。甜品或水果最多一段，并自动放在每条成片最后。拖动裁剪入点、出点后点击“确定所选片段”，已确认的裁剪会复用于后续方案。</p><div className="clip-library-status"><span className="muted">自动检查每 30 秒 · 最近扫描 {syncTime}</span>{clipsLoadError && <span className="clip-sync-error">扫描失败：{clipsLoadError}</span>}</div></div><div className="batch-control-grid"><label className="field"><span>批量合成数量</span><input className="input" type="number" min="1" max="20" value={batchCount} onChange={event => setBatchCount(Number(event.target.value))} /></label><label className="field"><span>每条片段数量</span><input className="input" type="number" min="1" max="20" value={clipCount} onChange={event => setClipCount(Number(event.target.value))} /></label><button type="button" className="btn btn-primary" onClick={recommend} disabled={candidates.filter(clip => clip.sourcePath).length === 0}>智能推荐方案</button><button type="button" className="btn" onClick={randomize} disabled={candidates.filter(clip => clip.sourcePath).length === 0}>随机生成方案</button><button type="button" className="btn" onClick={composeAll} disabled={busy}>{busy ? "批量合成中..." : "批量开始合成"}</button></div></section><div className="compose-workspace-list">{workspaces.map(workspace => <WorkspaceCard key={workspace.id} workspace={workspace} active={workspace.id === activeWorkspaceId} onSelect={() => setActiveWorkspace(workspace.id)} candidates={candidates} dragging={dragging} setDragging={setDragging} reorderWorkspace={reorderWorkspace} removeWorkspaceClip={removeWorkspaceClip} addWorkspaceClip={addWorkspaceClip} updateWorkspaceClip={updateWorkspaceClip} preflight={preflightReports[workspace.id] ?? null} preflightBusy={preflightBusy === workspace.id} onPreflight={() => { void checkWorkspace(workspace.id); }} compose={async () => { setBusy(true); try { await composeWorkspace(workspace.id); onToast(`${workspace.title} 已完成`); } catch (error) { onToast(`合成失败：${error instanceof Error ? error.message : "未知错误"}`); } finally { setBusy(false); } }} busy={busy} />)}</div><div className="compose-actions"><button type="button" className="btn" onClick={() => navigate("/workflow/sound")}>合成后配置声音文字</button></div></main>;
}
