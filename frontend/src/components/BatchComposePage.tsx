...
import { isTrimConfirmed, resolveDishCategory, type ComposeWorkspace, type TimelineClip } from "../model";
...
export function BatchComposePage({ onToast }: Props) {
  ...
  const usableCandidates = candidates.filter(clip => Boolean(clip.generatorNodeId && clip.sourcePath && clip.isSelected !== false));
  ...
  return <main className="step-main"><div className="step-breadcrumb"><button type="button" className="link-button" onClick={() => navigate("/")}>工作台首页</button><span>/</span><strong>成片合成</strong></div><div className="step-header"><StepHeading route="/workflow/compose" /></div><div className="step-guide"><span>操作提示</span><p>先点击“智能推荐方案”或“随机生成方案”，再在每条时间轴中拖动片段、调整入点和出点。裁剪确认后会被记住，后续方案可继续复用。</p></div><section className="step-panel batch-compose-controls"><div><span className="panel-label">BATCH COMPOSE</span><h2>{batchCount} 条成片 · 每条 {clipCount} 个片段</h2><p className="muted">当前版本候选片段 {usableCandidates.length} 个；甜品或水果最多一段，并自动放在每条成片最后。{usableCandidates.length === 0 ? "请先在第 4 步生成并下载真实 MP4。" : ""}</p><div className="clip-library-status"><span className="muted">自动检查每 30 秒 · 最近扫描 {syncTime}</span>{clipsLoadError && <span className="clip-sync-error">扫描失败：{clipsLoadError}</span>}</div></div><div className="batch-control-grid"><label className="field"><span>批量合成数量</span><input className="input" type="number" min="1" max="20" value={batchCount} onChange={event => setBatchCount(Number(event.target.value))} /></label><label className="field"><span>每条片段数量</span><input className="input" type="number" min="1" max="20" value={clipCount} onChange={event => setClipCount(Number(event.target.value))} /></label><button type="button" className="btn btn-primary" onClick={recommend} disabled={!usableCandidates.length}>智能推荐方案</button><button type="button" className="btn" onClick={randomize} disabled={!usableCandidates.length}>随机生成方案</button><button type="button" className="btn" onClick={composeAll} disabled={busy}>{busy ? "批量合成中..." : "批量开始合成"}</button></div></section><div className="compose-workspace-list">{workspaces.map(workspace => <WorkspaceCard key={workspace.id} workspace={workspace} active={workspace.id === activeWorkspaceId} onSelect={() => setActiveWorkspace(workspace.id)} candidates={usableCandidates} dragging={dragging} setDragging={setDragging} reorderWorkspace={reorderWorkspace} removeWorkspaceClip={removeWorkspaceClip} addWorkspaceClip={addWorkspaceClip} updateWorkspaceClip={updateWorkspaceClip} preflight={preflightReports[workspace.id] ?? null} preflightBusy={preflightBusy === workspace.id} onPreflight={() => { void checkWorkspace(workspace.id); }} compose={async () => { setBusy(true); try { await composeWorkspace(workspace.id); onToast(`${workspace.title} 已完成`); } catch (error) { onToast(`合成失败：${error instanceof Error ? error.message : "未知错误"}`); } finally { setBusy(false); } }} busy={busy} />)}</div><div className="compose-actions"><button type="button" className="btn" onClick={() => navigate("/workflow/sound")}>合成后配置声音文字</button></div></main>;
}

function WorkspaceCard({ workspace, active, onSelect, candidates, dragging, setDragging, reorderWorkspace, removeWorkspaceClip, addWorkspaceClip, updateWorkspaceClip, preflight, preflightBusy, onPreflight, compose, busy }: { workspace: ComposeWorkspace; active: boolean; onSelect: () => void; candidates: TimelineClip[]; dragging: string | null; setDragging: (id: string | null) => void; reorderWorkspace: (workspaceId: string, sourceId: string, targetId: string) => void; removeWorkspaceClip: (workspaceId: string, clipId: string) => void; addWorkspaceClip: (workspaceId: string, clipId: string) => void; updateWorkspaceClip: (workspaceId: string, clipId: string, patch: Partial<TimelineClip>) => void; preflight: PreflightReport | null; preflightBusy: boolean; onPreflight: () => void; compose: () => Promise<void>; busy: boolean }) {
  const missing = workspace.clips.length === 0 || workspace.clips.some(clip => !clip.sourcePath);
  return <section className={`step-panel compose-workspace-card ${active ? "is-active" : ""}`} onClick={onSelect}>
    <div className="panel-section-head"><div><span className="panel-label panel-note">成片方案</span><h2>{workspace.title} · {workspace.clips.length} 个片段</h2></div><span className={`source-status ${missing ? "pending" : "ready"}`}>{missing ? "待补齐" : "可合成"}</span></div>
    <StoryboardTimeline clips={workspace.clips} mode="clip" overlayItems={[]} onUpdateClip={(clipId, patch) => updateWorkspaceClip(workspace.id, clipId, patch)} />
    <div className="ordered-clip-list">{workspace.clips.map((clip, index) => <div className="ordered-clip" key={clip.id} draggable onDragStart={() => setDragging(`${workspace.id}:${clip.id}`)} onDragOver={event => event.preventDefault()} onDrop={() => { if (dragging?.startsWith(`${workspace.id}:`)) reorderWorkspace(workspace.id, dragging.slice(workspace.id.length + 1), clip.id); setDragging(null); }}><span className="drag-handle">⠿</span><span className="order-number">{index + 1}</span>{clip.sourceUrl ? <video className="clip-list-thumb" preload="metadata" src={clip.previewUrl ?? clip.sourceUrl} /> : null}<div><strong>{clip.dish || "未命名菜品"}</strong><small>{clip.label} · {resolveDishCategory(clip)} · 质量 {clip.qualityScore ?? "-"}/100 · {clip.timelineDuration.toFixed(1)}s · 源 {Number(clip.sourceStartSeconds ?? 0).toFixed(1)}-{Number(clip.sourceEndSeconds ?? clip.sourceDurationSeconds ?? clip.timelineDuration).toFixed(1)}s · {isTrimConfirmed(clip) ? "裁剪已保存" : "待确认裁剪"} · {clip.sourcePath ? "已关联真实文件" : "待生成"}</small></div><button type="button" className="clip-remove" aria-label={`移除${clip.dish}`} onClick={() => removeWorkspaceClip(workspace.id, clip.id)}>×</button></div>)}</div>
    <div className="compose-pool"><span className="muted">补入候选片段</span><div className="pool-grid">{candidates.filter(clip => !workspace.clips.some(item => item.id === clip.id)).map(clip => <button type="button" className="pool-card" key={clip.id} onClick={() => addWorkspaceClip(workspace.id, clip.id)}><span>{clip.dish || "未命名菜品"} · {resolveDishCategory(clip)} · {clip.label}</span><small>{clip.sourcePath ? `质量 ${clip.qualityScore ?? "-"}/100 · 加入此方案${clip.qualityWarnings?.length ? ` · ${clip.qualityWarnings.length} 条质量提示` : ""}` : "待生成，暂不可合成"}</small></button>)}</div></div>
    ...
    <div className="compose-actions"><button type="button" className="btn" disabled={preflightBusy || busy} onClick={onPreflight}>{preflightBusy ? "预检中..." : "运行预检"}</button><button type="button" className="btn btn-primary" disabled={busy || missing} onClick={compose}>合成此条</button>{workspace.job ? <span className="muted">{workspace.job.status === "running" || workspace.job.status === "queued" ? workspace.job.stage || "合成中..." : workspace.job.status === "done" ? "已完成" : workspace.job.error}</span> : null}</div>{workspace.job?.output_url ? <video className="workspace-result-video" controls preload="metadata" src={workspace.job.output_url} /> : null}
  </section>;
}
