...
import { ComposePage, GeneratorPage, OutputPage, StepPage, TimelinePage } from "./components/StepPages";
...
function useWorkflowPath(): WorkflowRoute {
  const [path, setPath] = useState<WorkflowRoute>(() => routeForPath(window.location.pathname));
  useEffect(() => {
    const update = () => setPath(routeForPath(window.location.pathname));
...
function RouteContent({ path, onToast }: { path: WorkflowRoute; onToast: (message: string) => void }) {
  ...
  if (path === "/workflow/timeline") return <TimelinePage onToast={onToast} />;
  if (path === "/workflow/compose") return <ComposePage onToast={onToast} />;
