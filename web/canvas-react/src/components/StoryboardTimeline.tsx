...
function TimelineRangeBlock({ className, label, tone, total: _total, style, removeLabel, onRemove, onClick, onPointerDown, onResizeStart, onResizeEnd }: { className: string; label: string; tone?: "voice"; total: number; style?: CSSProperties; removeLabel?: string; onRemove?: () => void; onClick?: () => void; onPointerDown?: (event: ReactPointerEvent<HTMLDivElement>) => void; onResizeStart?: (event: ReactPointerEvent<HTMLButtonElement>) => void; onResizeEnd?: (event: ReactPointerEvent<HTMLButtonElement>) => void }) {
  return <div className={`storyboard-range-block ${className}`} style={style} role="button" tabIndex={0} onPointerDown={onPointerDown} onClick={onClick}>
    <button type="button" className="storyboard-range-handle range-handle-start" aria-label="Resize start" onPointerDown={onResizeStart} />
    ...
    <button type="button" className="storyboard-range-handle range-handle-end" aria-label="Resize end" onPointerDown={onResizeEnd} />
  </div>;
}
