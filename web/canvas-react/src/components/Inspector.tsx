  ...
  return <>
    ...
    {activePanel === "overlay" ? <>
      ...
      <div className="overlay-editor-list">{overlayItems.map((item, index) => { const style = overlayStyleFromItem(item); return <div className="overlay-editor-item" key={item.id}>
        ...
        <div className="field-grid"><Field label="文字效果"><Select value={item.animation === "typewriter" ? "打字机" : "静态"} options={["静态", "打字机"]} onChange={value => updateOverlay(item.id, { animation: value === "打字机" ? "typewriter" : "static" })} /></Field><label className="field"><span>跟随人声</span><select className="input" value={item.syncVoiceId || "none"} onChange={event => updateOverlay(item.id, { syncVoiceId: event.target.value === "none" ? undefined : event.target.value })}><option value="none">不绑定</option>{voiceItems.map((voice, voiceIndex) => <option value={voice.id} key={voice.id}>人声 {voiceIndex + 1} · {voice.text || "未填写"}</option>)}</select></label></div>
...
function TypeFields({ node, onToast }: { node: WorkflowNode; onToast: (message: string) => void }) {
  ...
  if (data.kind === "image_process") return <><SectionTitle>图片处理参数</SectionTitle><Field label="背景虚化"><input className="input" type="number" min="0" max="24" step="1" value={data.backgroundBlur ?? 4} onChange={event => update({ backgroundBlur: Number(event.target.value) || 0 })} /></Field><Field label="背景亮度"><input className="input" type="number" min="0.35" max="1" step="0.05" value={data.backgroundBrightness ?? 0.72} onChange={event => update({ backgroundBrightness: Number(event.target.value) || 0.72 })} /></Field><Field label="主体大小"><input className="input" type="number" min="0.2" max="1" step="0.05" value={data.subjectScale ?? 0.68} onChange={event => update({ subjectScale: Number(event.target.value) || 0.68 })} /></Field><div className="preview-box">背景模板、主体位置和执行处理在“图片处理”独立页面完成。{data.processedImageName ? ` 当前结果：${data.processedImageName}` : " 尚未生成处理后首帧。"}</div></>;
