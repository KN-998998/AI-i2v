...
@router.get("/api/canvas/asset-library/classifications")
...
@router.put("/api/canvas/asset-library/manual-review/scans/{scan_id}/state")
def put_manual_asset_review_state(scan_id: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    request = payload or {}
    try:
        return save_manual_review_state(scan_id, request.get("selections", {}), request.get("excluded_dish_keys", []))
    except (OSError, ValueError) as exc:
        raise _json_error(str(exc), 400) from exc
