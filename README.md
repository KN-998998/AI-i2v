...
- 周计划生产可按日自动选材建稿并定时执行，候选片段全部经人工审核后才进入合成
...
| 素材库整理 | 人工确认分类，把零散菜品图整理为「分类/菜品/图片」目录 | 已实现 |
| 图片处理 | GoodsMatting 抠图、背景模板、首帧预览台与参数重合成 | 需配置云服务 |
...
| 任务中心 | 集中查看各任务阶段、失败原因与重试状态 | 已实现 |
| 周计划生产 | 按日自动选材建稿、定时执行、相邻三日菜品不重复 | 已实现 |
| OSS 素材抽取 | ECS RAM 角色只读 OSS、按分类随机选取菜品并标准化为 9:16 | 已实现 |
| 片段审核 | 逐条通过/淘汰候选片段并确认裁剪，构成合成前置门禁 | 已实现 |
...
周计划按天创建独立草稿，自动完成选材、图片处理与片段生成，随后进入**强制人工审核**：每个已生成片段都必须标记为通过或淘汰，通过时确认裁剪区间。只有当通过数量不少于每条成片所需片段数时，才允许进入无声成片组合。
...
第 2 步采用双栏布局：左侧是固定的首帧预览，右侧是背景模板和五个调节滑块（背景虚化、背景亮度、菜品大小、水平位置、垂直位置）。改动模板或松开滑块后会自动重新合成，无需回到页面顶部再点一次按钮。

抠图结果会留档复用，因此调整滑块时只做合成、不再重复抠图；需要重做抠图时用“重新抠图”。对于改动上线前处理、节点上没有留档抠图的旧草稿，第一次调整会先自动补抠一次（约半分钟，会有提示），之后再调整即为即时更新。

### 为什么成片比参考片暗

拿量参考片的同一把尺子量工具合成的成片，画面平均亮度只有 **65.7**，而 11 条已发布的
参考片是 **109 / 117 / 123**（p10 / 中位 / p90），差了快一倍。

成因之一是第 2 步默认把背景压暗：这个默认值原来是 0.72，现在调到了 **0.85**。但要如实
说清楚——只调这个默认值，预计成片平均亮度到 **74 左右，到不了 109**。

剩下的差距来自背景素材本身偏暗（门店实拍的深色木桌），**要换素材才能补上，不是代码能
解决的**。工具这边没有再给成片加一道亮度归一化滤镜：硬拉亮度会不会把深色木桌拉成灰、
把菜品拉过曝，得先看过实物再定。

老草稿里存着的旧默认值 0.72 会在打开时自动改成 0.85（只改正好等于 0.72 的，特意调过的
数值不碰）。但**已经用 0.72 处理好的首帧不会自动重做**：想让它跟上，在第 2 步随便碰一下
滑块，松手后会重新合成一次。
...
在“周计划生产”中指定菜品素材库、背景素材库、起始日期、连续天数、每日执行时间和一个画布模板草稿，再为每天设置候选片段数、成片数和每条成片片段数。系统会按分类配额自动选材，并保证相邻三天的菜品不重复。

- 计划可暂停、恢复或取消；暂停和取消不会删除已产生的草稿与片段。
- 到点后自动建稿并推进图片处理与片段生成，进度在“任务中心”查看。
- 生成完成后进入“候选片段审核”，逐条通过或淘汰并确认裁剪；全部处理完且通过数量达标后，才能进入第 5 步合成。
- 片段审核只对周计划运行生效，手工路径仍在成片合成页内确认裁剪。
...
frontend/                 React + TypeScript + Vite + React Flow UI
  ...
  src/workflowProgress.ts 步骤顺序解锁与进度推导
  src/tutorial.ts         内置分步教程内容
web/                      FastAPI 应用、API 与服务编排
  api/                    HTTP 路由与请求校验
  services/               草稿、素材、图片处理、生成、合成与质量服务
    weekly_plans.py       周计划、素材预留与定时执行
    task_contract.py      长任务状态契约与重试策略
  core/                   设置与日志
...
- 周计划、素材预留和片段审核记录保存在 `output/weekly_plans.sqlite3`；每日运行生成的草稿仍复用同一套草稿与片段库。
- 外部生成任务取得 `task_id` 后会持久化，后端重启后可继续轮询。
- 长任务共用统一状态契约（排队、处理、轮询、下载、评分、重试、完成、失败）；本地阶段失败会按策略有限重试，外部 Kling 任务不会被重复提交。
